...
#-----------------------------------------------------------------------------------------------------------------------------#
# 
# This script processes a csv file with data in columns (lon,lat,depth,value) and uses GMT to plot:
#   1) A map showing the cross section location
#   2) The cross section projected from lon/lat to meters
#
# This is used by the CVM explorer to plot CVM data.
#  
#   Usage: ./plotCVM-vertSection.pl file.csv
# 
#-----------------------------------------------------------------------------------------------------------------------------#
...
$distFile=substr($csvFile,0,-4).".dist";
...
#-----------------------------------------------------------------------------------------------------------------------------#
if($plotVels>0){
	#print "Running blockmean to prevent aliasing because the cross section spacing is not even because it was converted from lon/lat to distance in meters\n";
	system "gmt blockmean $distFile $Rext -I$xRes/$yRes -i4,2,3 > $meanFile";
	print "Interpolating data using GMT's surface\n";
	system "gmt surface $meanFile $Rext -Gz.grd -I$xRes/$yRes -T$t -M7c -Ll0 -rg";
}
...
system "gmt grdimage z.grd -X8.5i -Y0.85i $Rxy -JX7.0i/-5.0i -Cz.cpt -O -K >> $plotFile";
...
system "gmt psscale -R -JX -Cz.cpt -B$cAxis+l\"$zTitle\" -Dx0/-0.85i+w7.0i/0.25i+jBL+h -O -K >> $plotFile";
